...
**现象**: 在消息、首页、我的发布、收藏等页面来回切换几次后，登录状态丢失，跳转到登录页
**原因**: `fetchUser()` 方法在任何请求失败时都会调用 `logout()`，网络波动或服务器重启时会错误清除登录状态
**修复**: 只在 401 错误时才调用 `logout()`，其他错误保留 token
